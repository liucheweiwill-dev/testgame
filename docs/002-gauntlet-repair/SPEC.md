# SPEC — 002 gauntlet repair (Tier 2), revision 2
...
green result — including whatever weak tests it exposes.
...
1. **No change under `src/`.** This task fixes the gauntlet, not the product. A
   diff touching `src/` means the scope was wrong.
